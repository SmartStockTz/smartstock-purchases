    ...
    >
          ...
          <div *ngFor="let cart of cartState.carts | async; let i = index">
            <mat-list-item matTooltip="{{ cart.product.product }}">
              ...
              <mat-card-subtitle matLine>
                {{ cart.quantity | number }} {{ cart.product.unit }} @
                {{ cart.purchase | number }} =
      ...
      >
        ...
        <div class="checkout-container">
          <button
            [disabled]="(purchaseState.addPurchasesProgress | async) === true"
            (click)="checkout()"
          ...
          >
            ...
            <mat-progress-spinner
              *ngIf="(purchaseState.addPurchasesProgress | async) === true"
              mode="indeterminate"
              diameter="25"
            ...
            <span
              style="float: right"
              *ngIf="(purchaseState.addPurchasesProgress | async) === false"
              >Record</span
            >
          </button>
...
export class PurchaseCartComponent implements OnInit, OnDestroy {
  ...
  checkout(): void {
    if (
      !this.cartState.selectedSupplier.value &&
