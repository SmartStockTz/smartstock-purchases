...
export class StockState {
  ...
  exportToExcel(): void {
    ...
    this.stockService
      ...
      .then((value) => {
        this.messageService.showMobileInfoMessage(
          'Stocks sent to your email, visit your email to download it',
          1000,
          'bottom'
        );
      })
  ...
  deleteStock(stock: StockModel): void {
    ...
    this.stockService
      ...
      .then(async (stocks) => {
        await this.storageService.saveStock(
          stocks.filter((x) => x.id !== stock.id) as any
        );
        return stocks;
      })
      .then((stocks) => {
  ...
  deleteManyStocks(selectionModel: SelectionModel<StockModel>): void {
    ...
    this.stockService
      .deleteMany(selectionModel.selected.map((x) => x.id))
      .then((_) => {
        this.messageService.showMobileInfoMessage(
          'Products deleted',
          2000,
        ...
        this.stocks.next(
          this.stocks.value.filter(
            (x) =>
              selectionModel.selected.findIndex((y) => y.id === x.id) === -1
          )
        );
